          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '6px' }}>
            ...
            <div style={{ paddingTop: '2px', position: 'relative' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                ...
                {isToday && <span style={{ fontSize: '8px', background: theme.pageHeaderBorder, color: theme.pageBg, borderRadius: '3px', padding: '0 4px', lineHeight: 1.5, position: 'absolute', right: 0, top: 0 }}>TODAY</span>}
              </div>
              <div style={{ fontSize: '9px', color: isWe ? theme.weekendColor : theme.subColor, marginTop: '1px' }}>
                {WDS[wd]}曜日
              </div>
            </div>
          </div>
...
export default function App() {
  ...
  useEffect(() => {
    localStorage.setItem('myd_base', base.toISOString());
    localStorage.setItem('myd_base_year', String(baseYear));
  }, [base, baseYear]);
