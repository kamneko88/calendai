...
**2026-09-18〜：回覧板T-010をL0からL1へ変更。コード実装はヘッドレス（`dev/_runner.ps1`）が基本。**
Play Console操作・実機確認など人の目が要る工程は引き続きあつのりさん本人が担当する。

このプロジェクトは以下の分担で進行します。

1. 統括のClaude（別チャット・あつのりさんと直接対話する担当）が設計・指示をまとめる。
   **「実装方針への承認」はこの段階で完了させる**（ヘッドレス起動後に途中で承認を求めることはしない）
2. 統括のClaudeが`dev/_runner.ps1`経由でClaude Codeをヘッドレス起動し、指示書どおりに実装させる。
   **Claude Code（あなた）は、方針確認だけで止まらないこと。これはヘッドレス実行で、
   あなたの質問に答える人はいません。** 指示書に穴がある（事実が足りない・矛盾している）
   ときだけ止まり、何が足りないかを報告に書くこと。バックグラウンドに投げたり、
   通知待ちでturnを終えたりしないこと
3. 実装後は動作確認（ビルド・テスト）の上、コミット・プッシュまでをClaude Code側で行う。
   **完了報告は`git show`等の差分で行い、ファイル全文の貼り付けはしないこと**
...
不明点・矛盾があれば、着手前に指示書の6節チェック（背景／スコープ／やること／禁止事項／
完了条件／コミットメッセージ）を行い、欠けていたら実装を始めず報告すること。
...
詳しい罠・チェックリストは`headless-code-runner`スキル（統括側が参照）を参照。

### 着手前の指示書チェック（回覧板 T-009）

**着手前に、指示書に6つの節が揃っているか確認すること。**
背景／スコープ／やること／禁止事項／完了条件／コミットメッセージ。
**1つでも欠けていたら実装を始めず「◯◯が見当たりません」と報告すること。**
